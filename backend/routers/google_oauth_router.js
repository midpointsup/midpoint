...
googleOAuthRouter.post('/', async (req, res) => {   
    try {
      ...
      const userResponse = await axios.get(
        ...
        if (!user) {
            user = await createUser(userResponse.data.email, userResponse.data.name, userResponse.data.picture);
            if (!user) {   
            return res.status(422).json({ error: user.error });
            }
        } else {
            user = await updateUserProfilePic(userResponse.data.email, userResponse.data.picture);
            if (!user) {
                console.log("Unable to update profile picture")
            }
        }
        const token = await storeGoogleToken(user, tokens.tokens.access_token, tokens.tokens.refresh_token);
        if (!token) {
        ...
        return res.status(200).json({ username: userResponse.data.name, email: userResponse.data.email, picture: userResponse.data.picture, token: accessToken.access_token });
    } catch (error) {

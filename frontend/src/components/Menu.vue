    ...
    <div class="column-display">
        <ul class="nav nav-pills nav-flush flex-column mb-auto text-center">
            <li @click="toggleSidebar" v-if="!isSidebarOpen">
                <a href="#" class="nav-link py-3" aria-current="page" title="" data-bs-toggle="tooltip"
            ...
            <li>
                <a @click="showPlans" title="Groups" href="#" class="nav-link link-dark side-bar-icon">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                        class="bi bi-people-fill icon" viewBox="0 0 16 16">
                        <path
            ...
            <li>
                <a @click="showCreatePlan" title="addPlan" href="#" class="nav-link link-dark side-bar-icon">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
        ...
        <div class="dropdown">
            <a href="#" class="d-flex align-items-center link-dark text-decoration-none" id="dropdownUser2"
                ...
                <img src="https://avatars.githubusercontent.com/u/57234754?s=400&u=7928d03725e3a137f7c37cd146c6cf0177b4765a&v=4"
        ...
        <div class="d-flex flex-column flex-shrink-0 p-3 bg-light" style="width: 280px; height: 100%">
            ...
            <div v-if="plansPage" class="nav nav-pills flex-column mb-auto">
                <div v-if="selectedPlan" class="nav nav-pills flex-column mb-auto">
                    ...
                    <button @click="clearSelection" class="btn btn-secondary mt-3">Back</button>
                </div>
                <div v-else>
                    <h6>My Plans</h6>
                    <ul class="nav nav-pills flex-column mb-auto">
            ...
            <div v-else-if="createPlan" class="nav nav-pills flex-column mb-auto">
                <h6>Create a Plan</h6>
                Plan Name
                <input class="form-control" type="text" v-model="newPlanName" placeholder="Enter plan name" autofocus />

                <br>
                Invite Members
                <input class="form-control" type="text" v-model="newMemberName" @keyup.enter="addMember"
                    placeholder="Add member" />

                <div class="mt-3">
                    <span class="badge badge-pill bg-primary" v-for="(member, index) in membersList" :key="index">
                        {{ member }} <button @click="removeMember(index)" class="btn btn-sm">x</button>
                    </span>
                </div>

                <br>
                Where do you want to meet?
                <input class="form-control" type="text" v-model="newLocation" placeholder="Enter location" autofocus />

                <br>
                Select Categories
                <div id="categoryContainer">
                    <div v-for="category in categories" :key="category.id" :data-key="category.id" class="category-icon"
                ...
                <br>
                Date
                <input class="form-control" type="text" placeholder="Enter date" autofocus />
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            showPlans() {
                if(!this.isSidebarOpen) {this.isSidebarOpen = true;}
                this.plansPage = true;
            ...
            toggleCategorySelection(categoryId) {
                ...
                if (index > -1) {
                    document.querySelector(`[data-key='${categoryId}']`).classList.remove('selected');
                    this.selectedCategoryIds.splice(index, 1);
                } else {
                    document.querySelector(`[data-key='${categoryId}']`).classList.add('selected');
